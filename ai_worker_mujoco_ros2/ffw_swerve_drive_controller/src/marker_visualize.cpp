...
void MarkerVisualize::init(rclcpp_lifecycle::LifecycleNode::WeakPtr node_interface_weak_ptr,
                           const std::string &base_frame_id, const std::string &marker_topic_name,
                           size_t num_modules, double visualization_update_period_sec)
{
    ...
    {
        marker_lifetime_ =
            rclcpp::Duration::from_seconds(visualization_update_period_sec * 1.2 + 0.05);
    }
    ...
    if (auto node = node_interface_.lock())
    {
        ...
        RCLCPP_DEBUG(node->get_logger(),
                     "MarkerVisualize initialized, publishing to: %s. Marker lifetime: %.2f s",
...
void MarkerVisualize::add_delete_marker_action(visualization_msgs::msg::MarkerArray &marker_array,
...
std::optional<Point2D> MarkerVisualize::get_line_intersection(Point2D p1, Point2D p2, Point2D p3,
                                                              Point2D p4)
{
...
void MarkerVisualize::publish_markers(const rclcpp::Time &stamp, double robot_vx, double robot_vy,
                                      double robot_wz, const std::vector<double> &module_x_offsets,
                                      ...
                                      const std::vector<double> &final_wheel_linear_vels)
{
    ...
    if (!node || !marker_publisher_)
    {
        return;
    }
    if (module_x_offsets.size() != num_modules_ || module_y_offsets.size() != num_modules_ ||
        actual_robot_frame_steering_angles.size() != num_modules_ ||
        final_wheel_linear_vels.size() != num_modules_)
    {
        RCLCPP_WARN_THROTTLE(node->get_logger(), *node->get_clock(), 1000,
                             "MarkerVisualize: Mismatch in module data sizes. Expected %zu.",
                             num_modules_);
        return;
    }
    ...
    robot_vel_start.x = 0.0;
    robot_vel_start.y = 0.0;
    robot_vel_start.z = 0.18;
    ...
    robot_vel_color.r = 1.0f;
    robot_vel_color.g = 0.0f;
    robot_vel_color.b = 0.0f;
    robot_vel_color.a = 1.0f;
    std_msgs::msg::ColorRGBA text_color_white;
    text_color_white.r = 1.0f;
    text_color_white.g = 1.0f;
    text_color_white.b = 1.0f;
    text_color_white.a = 1.0f;
    std_msgs::msg::ColorRGBA text_color_blue;
    text_color_blue.r = 0.0f;
    text_color_blue.g = 0.0f;
    text_color_blue.b = 1.0f;
    text_color_blue.a = 1.0f;
    std_msgs::msg::ColorRGBA text_color_yellow;
    text_color_yellow.r = 1.0f;
    text_color_yellow.g = 1.0f;
    text_color_yellow.b = 0.0f;
    text_color_yellow.a = 1.0f;

    marker_array.markers.push_back(create_arrow_marker(ROBOT_VEL_ARROW_ID, stamp,
                                                       "robot_velocity_info", robot_vel_start,
                                                       robot_vel_end, robot_vel_color));

    std::ostringstream robot_vel_ss;
    robot_vel_ss << std::fixed << std::setprecision(2) << "[cmd_vel]" << "\n vx: " << robot_vx
                 << "\n vy: " << robot_vy << "\n wz: " << robot_wz;
    marker_array.markers.push_back(
        create_text_marker(ROBOT_VEL_TEXT_ID, stamp, "robot_velocity_info_text", robot_vel_ss.str(),
                           robot_text_pos, text_color_blue, MarkerVisualize::ROBOT_TEXT_SCALE));

    // --- 2.visualize each module's ICR and vector ---
    std_msgs::msg::ColorRGBA module_vel_color;
    module_vel_color.r = 0.0f;
    module_vel_color.g = 1.0f;
    module_vel_color.b = 0.0f;
    module_vel_color.a = 1.0f;
    std_msgs::msg::ColorRGBA steering_cube_color;
    steering_cube_color.r = 1.0f;
    steering_cube_color.g = 1.0f;
    steering_cube_color.b = 0.0f;
    steering_cube_color.a = 0.7f;
    std_msgs::msg::ColorRGBA wheel_perpendicular_line_color;
    wheel_perpendicular_line_color.r = 0.6f;
    wheel_perpendicular_line_color.g = 0.6f;
    wheel_perpendicular_line_color.b = 0.6f;
    wheel_perpendicular_line_color.a = 1.0f;
    ...
    if (num_modules_ == 4)
    {
        ...
        p.y = 0.4;
        p.z = 0.2;
        module_text_offsets_storage.push_back(p); // FL
        p.x = 0.4;
        p.y = 0.4;
        p.z = 0.2;
        module_text_offsets_storage.push_back(p); // FR
        p.x = -0.4;
        p.y = -0.4;
        p.z = 0.2;
        module_text_offsets_storage.push_back(p); // RL
        p.x = 0.4;
        p.y = -0.4;
        p.z = 0.2;
        module_text_offsets_storage.push_back(p); // RR
    }
    else if (num_modules_ == 3)
    {
        ...
        p.y = -0.5;
        p.z = 0.2;
        module_text_offsets_storage.push_back(p); // RC
    }
    ...
    for (size_t i = 0; i < num_modules_; ++i)
    {
        ...
        marker_array.markers.push_back(create_arrow_marker(
            ...
            module_vel_color, MarkerVisualize::ARROW_SHAFT_DIAMETER * 0.9,
            MarkerVisualize::ARROW_HEAD_DIAMETER * 0.9, MarkerVisualize::ARROW_HEAD_LENGTH * 0.9));

        std::ostringstream module_info_ss;
        module_info_ss << std::fixed << std::setprecision(1)
                       << "Steering:" << (robot_frame_steering_angle * 180.0 / M_PI)
                       << ",Wheel:" << std::fixed << std::setprecision(2) << wheel_lin_vel;
        ...
        marker_array.markers.push_back(
            create_cube_marker(MODULE_STEERING_CUBE_BASE_ID + i, stamp, "steering_directions",
        ...
        line_p1_viz.x =
            module_origin.x - viz_line_half_length * std::cos(perpendicular_to_steering_angle);
        line_p1_viz.y =
            module_origin.y - viz_line_half_length * std::sin(perpendicular_to_steering_angle);
        ...
        marker_array.markers.push_back(create_line_strip_marker(
            WHEEL_PERPENDICULAR_LINE_BASE_ID + i, stamp, "wheel_perpendicular_lines",
            line_points_viz, wheel_perpendicular_line_color, viz_line_thickness));
    }
    ...
    if (target_icr_valid)
    {
        ...
        marker_array.markers.push_back(
            create_sphere_marker(TARGET_ICR_POINT_ID, stamp, "icr_visualization", target_icr_point,
                                 MarkerVisualize::SPHERE_RADIUS * 0.9, text_color_blue));

        std_msgs::msg::ColorRGBA line_to_icr_color;
        line_to_icr_color.r = 0.7;
        line_to_icr_color.g = 0.7;
        line_to_icr_color.b = 0.0;
        ...
        for (size_t i = 0; i < num_modules_; ++i)
        {
            ...
            double icr_to_robot_dist = std::hypot(module_origin.x - target_icr_point.x,
    ...
    else
    {
        ...
        for (size_t i = 0; i < num_modules_; ++i)
        {
            add_delete_marker_action(marker_array, MODULE_LINE_TO_ICR_BASE_ID + i,
        ...
        for (size_t i = 0; i < num_modules_; ++i)
        {
            for (size_t j = i + 1; j < num_modules_; ++j)
            {
                std::optional<Point2D> intersection =
                    get_line_intersection(line_start_points[i], line_end_points[i],
    ...
    intersection_color.r = 0.0f;
    intersection_color.g = 1.0f;
    intersection_color.b = 1.0f;
    ...
    if (num_modules_ >= 2)
    {
        max_possible_intersections = num_modules_ * (num_modules_ - 1) / 2;
    }
    for (int k = 0; k < max_possible_intersections; ++k)
    {
        add_delete_marker_action(marker_array, INTERSECTION_POINT_BASE_ID + k,
    ...
    if (!intersection_points_2d.empty())
    {
        for (size_t k = 0; k < intersection_points_2d.size(); ++k)
        {
            ...
            marker_array.markers.push_back(create_sphere_marker(
                INTERSECTION_POINT_BASE_ID + k, stamp, "actual_icr_intersections", p_marker,
                intersection_radius, intersection_color));
...
visualization_msgs::msg::Marker MarkerVisualize::create_arrow_marker(
    ...
    const std_msgs::msg::ColorRGBA &color, double scale_x, double scale_y, double scale_z)
{
    ...
    double scale_x)
{
    ...
    line_strip.header.frame_id = base_frame_id_;
    ...
    line_strip.lifetime = marker_lifetime_;
    return line_strip;
}

visualization_msgs::msg::Marker MarkerVisualize::create_sphere_marker(
    int id, const rclcpp::Time &stamp, const std::string &ns,
    const geometry_msgs::msg::Point &position, double radius, const std_msgs::msg::ColorRGBA &color)
{
    visualization_msgs::msg::Marker sphere;
    sphere.header.frame_id = base_frame_id_;
    ...
    sphere.color = color;
    sphere.lifetime = marker_lifetime_;
    return sphere;
}

visualization_msgs::msg::Marker MarkerVisualize::create_text_marker(
    int id, const rclcpp::Time &stamp, const std::string &ns, const std::string &text,
    const geometry_msgs::msg::Point &position, const std_msgs::msg::ColorRGBA &color,
    double scale_z)
{
...
visualization_msgs::msg::Marker MarkerVisualize::create_cube_marker(
    int id, const rclcpp::Time &stamp, const std::string &ns, const geometry_msgs::msg::Pose &pose,
    const geometry_msgs::msg::Vector3 &scale, const std_msgs::msg::ColorRGBA &color)
{
    visualization_msgs::msg::Marker cube;
    cube.header.frame_id = base_frame_id_;
    ...
    cube.scale = scale;
    cube.color = color;
    cube.lifetime = marker_lifetime_;
    return cube;
}
