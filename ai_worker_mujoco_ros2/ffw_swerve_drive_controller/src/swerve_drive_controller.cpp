...
constexpr double ScaleWheelVelocityForAlignment(
  const double wheel_velocity, const bool is_aligned, const double alignment_factor)
{
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  try {
    ...
    wheel_velocity_during_alignment_factor_ = get_node()->get_parameter(
      "wheel_velocity_during_alignment_factor").as_double();
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  for (size_t i = 0; i < num_modules_; ++i) {
    ...
    try {
      ...
      final_wheel_vel_cmd = ScaleWheelVelocityForAlignment(
        final_wheel_vel_cmd, module_steering_aligned,
