...
void Odometry::init(const rclcpp::Time &time, const std::array<double, 3> &base_frame_offset)
{
    ...
    linear_x_accumulator_ =
        rcpputils::RollingMeanAccumulator<double>(velocity_rolling_window_size_);
    linear_y_accumulator_ =
        rcpputils::RollingMeanAccumulator<double>(velocity_rolling_window_size_);
    angular_z_accumulator_ =
        rcpputils::RollingMeanAccumulator<double>(velocity_rolling_window_size_);
}

void Odometry::setModuleParams(const std::vector<double> &module_x_offsets,
                               ...
                               const double wheel_radius)
{
    if (module_x_offsets.size() != module_y_offsets.size())
    {
        throw std::runtime_error(
            "Odometry: Module X and Y offset vectors must have the same size.");
    }
                      ...
                      const std::vector<double> &wheel_velocities, const double dt)
{
    ...
    if (solver_method_ == OdomSolverMethod::PSEUDO_INVERSE)
    {
        ...
        else
        {
            ...
            robot_twist_eigen(0) =
                invATA[0][0] * ATb[0] + invATA[0][1] * ATb[1] + invATA[0][2] * ATb[2];
            robot_twist_eigen(1) =
                invATA[1][0] * ATb[0] + invATA[1][1] * ATb[1] + invATA[1][2] * ATb[2];
            robot_twist_eigen(2) =
                invATA[2][0] * ATb[0] + invATA[2][1] * ATb[1] + invATA[2][2] * ATb[2];
        }
    }
    ...
    else
    {
        ...
        robot_twist_eigen =
            A_eigen.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(b_eigen);
    }
    // ****************************************
    ...
    tf2::Vector3 velocity_in_base_frame(velocity_in_base_frame_linear_x_,
                                        velocity_in_base_frame_linear_y_, 0.0);
